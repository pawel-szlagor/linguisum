...
public class ImportedDataIntegrationIntTest {
    ...
    public void shouldImportDataConsolidately() {
        ...
        // then
        assertThat(snapshot.getWeatherConditions(),
                equalTo(mapperFacade.map(weatherConditionsService.findByDateTime(testTime), EnvironmentConditions.class)));
        ...
        deviceStateSourceDtos.stream()
                             .map(mapDeviceStateFunction())
                             .forEach(l -> assertThat(snapshot.getDeviceStates(), hasItem(equalTo(l))));
        ...
        desiredTempSourceDtos.stream().map(desiredTempSourceDtoRoomStateFunction()).forEach(
                l -> assertThat(snapshot.getRoomStates(), hasItem(equalTo(l))));
        ...
        mediaUsageSourceDtos.stream().map(mediaUsageSourceDtoMediaUsageFunction()).forEach(
                l -> assertThat(snapshot.getMediaUsages(), hasItem(equalTo(l))));
        ...
        personPositionSourceDtos.stream().map(personPositionSourceDtoPersonStateFunction()).forEach(
                l -> assertThat(snapshot.getPersonStates(), hasItem(equalTo(l))));
    }

    private Function<MediaUsageSourceDto, MediaUsage> mediaUsageSourceDtoMediaUsageFunction() {
        return l -> MediaUsage.builder()
                              .location(mapperFacade.map(l.getLocation(), Room.class))
                              .mediaType(l.getMediaType())
                              .usagePerMinute(l.getUsagePerMinute())
                              .build();
    }
    ...
    private Function<DesiredTempSourceDto, RoomState> desiredTempSourceDtoRoomStateFunction() {
        return l -> RoomState.builder()
                             .desiredTemp(l.getDesiredTemp())
                             .person(mapperFacade.map(l.getUser(), Person.class))
                             .room(mapperFacade.map(l.getLocation(), Room.class))
                             .build();
    }
    ...
    private MapperFacade initializeMapperFacade() {
        ...
        factory.classMap(SnapshotSourceDto.class, Snapshot.class).byDefault().register();
        factory.classMap(WeatherConditionSourceDto.class, EnvironmentConditions.class).byDefault().register();
        factory.classMap(RoomSourceDto.class, Room.class).byDefault().register();
        factory.classMap(DeviceStateSourceDto.class, DeviceState.class).byDefault().register();
